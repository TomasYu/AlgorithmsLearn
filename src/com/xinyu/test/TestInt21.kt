...
class TestInt21 {
    ...
    fun countArrangement(n: Int): Int {
        ...
        return helper(n, 1, flag)

    }

    private fun helper(n: Int, cur: Int, flag: Array<Boolean>): Int {
        if (cur > n){
            return 1
        }
        var result = 0
        for (i in 1..n){
            if (!flag[i] && (cur % i == 0 || i % cur == 0)){
                flag[i] = true
                result += helper(n,cur+1,flag)
                flag[i] = false
            }
        }
        return result
...
fun main() {
    println(TestInt21().countArrangement(3))
}